...
{
public:
    explicit ArrowWidget(vec3 direction);
    bool draw();
...
namespace ImGuiExt {
    ...
    bool DirectionWidget(const char* label, float v[3]) 
    {
        ...
        // Dragvec3 is nicer than Slidervec3 because it supports double-clicking for keyboard entry.
...
ArrowWidget::ArrowWidget(vec3 direction) 
{
    quatFromDirection(mDirectionQuat, direction);
}

vec3 ArrowWidget::getDirection() const 
{
    vec3 d = mDirectionQuat * vec3(1, 0, 0);
    return d / length(d);
}
...
bool ArrowWidget::draw() 
{
    ...
    static quat origQuat;
    static vec3 coordOld;
    ...
    if (ImGui::IsItemActive()) 
    {
        ...
        if (ImGui::IsMouseClicked(0)) 
        {
            ...
            coordOld = vec3(quatIX((int)mouse.x, w, h), quatIY((int)mouse.y, w, h), 1.0f);
        }
        else if (ImGui::IsMouseDragging(0)) 
        {
            vec3 coord(quatIX((int)mouse.x, w, h), quatIY((int)mouse.y, w, h), 1.0f);
            vec3 pVec = coord;
            vec3 oVec = coordOld;
            coord.z = 0.0f;
            ...
            if (n0 > FLT_EPSILON && n1 > FLT_EPSILON) 
            {
                ...
                quat qrot, qres, qorig;
                qrot = angleAxis(angle, axis);
    ...
    quat quat = normalize(mDirectionQuat);
    ImColor alpha(1.0f, 1.0f, 1.0f, highlighted ? 1.0f : 0.75f);
    vec3 arrowDir = quat * vec3(1, 0, 0);
    ...
    for (int k = 0; k < 4; ++k) 
    {
        ...
        {
            vec3 coord = s_ArrowTri[j][i];
            vec3 norm = s_ArrowNorm[j][i];
            if (coord.x > 0) {
...
void ArrowWidget::createArrow() 
{
    ...
    for (int i = 0; i < SUBDIV; ++i) 
    {
        ...
        y1 = cosf(a1);
        z1 = sinf(a1);
        s_ArrowTri[ARROW_CYL].push_back(vec3(x1, CYL_RADIUS * y0, CYL_RADIUS * z0));
        s_ArrowTri[ARROW_CYL].push_back(vec3(x0, CYL_RADIUS * y0, CYL_RADIUS * z0));
        s_ArrowTri[ARROW_CYL].push_back(vec3(x0, CYL_RADIUS * y1, CYL_RADIUS * z1));
        s_ArrowTri[ARROW_CYL].push_back(vec3(x1, CYL_RADIUS * y0, CYL_RADIUS * z0));
        s_ArrowTri[ARROW_CYL].push_back(vec3(x0, CYL_RADIUS * y1, CYL_RADIUS * z1));
        s_ArrowTri[ARROW_CYL].push_back(vec3(x1, CYL_RADIUS * y1, CYL_RADIUS * z1));
        s_ArrowNorm[ARROW_CYL].push_back(vec3(0, y0, z0));
        s_ArrowNorm[ARROW_CYL].push_back(vec3(0, y0, z0));
        s_ArrowNorm[ARROW_CYL].push_back(vec3(0, y1, z1));
        s_ArrowNorm[ARROW_CYL].push_back(vec3(0, y0, z0));
        s_ArrowNorm[ARROW_CYL].push_back(vec3(0, y1, z1));
        s_ArrowNorm[ARROW_CYL].push_back(vec3(0, y1, z1));
        s_ArrowTri[ARROW_CYL_CAP].push_back(vec3(x0, 0, 0));
        s_ArrowTri[ARROW_CYL_CAP].push_back(vec3(x0, CYL_RADIUS * y1, CYL_RADIUS * z1));
        s_ArrowTri[ARROW_CYL_CAP].push_back(vec3(x0, CYL_RADIUS * y0, CYL_RADIUS * z0));
        s_ArrowNorm[ARROW_CYL_CAP].push_back(vec3(-1, 0, 0));
        s_ArrowNorm[ARROW_CYL_CAP].push_back(vec3(-1, 0, 0));
        s_ArrowNorm[ARROW_CYL_CAP].push_back(vec3(-1, 0, 0));
        ...
        s_ArrowTri[ARROW_CONE].push_back(vec3(x1, 0, 0));
        s_ArrowTri[ARROW_CONE].push_back(vec3(x0, CONE_RADIUS * y0, CONE_RADIUS * z0));
        s_ArrowTri[ARROW_CONE].push_back(vec3(x0, CONE_RADIUS * y1, CONE_RADIUS * z1));
        s_ArrowTri[ARROW_CONE].push_back(vec3(x1, 0, 0));
        s_ArrowTri[ARROW_CONE].push_back(vec3(x0, CONE_RADIUS * y1, CONE_RADIUS * z1));
        s_ArrowTri[ARROW_CONE].push_back(vec3(x1, 0, 0));
        s_ArrowNorm[ARROW_CONE].push_back(vec3(nn * nx, nn * y0, nn * z0));
        s_ArrowNorm[ARROW_CONE].push_back(vec3(nn * nx, nn * y0, nn * z0));
        s_ArrowNorm[ARROW_CONE].push_back(vec3(nn * nx, nn * y1, nn * z1));
        s_ArrowNorm[ARROW_CONE].push_back(vec3(nn * nx, nn * y0, nn * z0));
        s_ArrowNorm[ARROW_CONE].push_back(vec3(nn * nx, nn * y1, nn * z1));
        s_ArrowNorm[ARROW_CONE].push_back(vec3(nn * nx, nn * y1, nn * z1));
        s_ArrowTri[ARROW_CONE_CAP].push_back(vec3(x0, 0, 0));
        s_ArrowTri[ARROW_CONE_CAP].push_back(vec3(x0, CONE_RADIUS * y1, CONE_RADIUS * z1));
        s_ArrowTri[ARROW_CONE_CAP].push_back(vec3(x0, CONE_RADIUS * y0, CONE_RADIUS * z0));
        s_ArrowNorm[ARROW_CONE_CAP].push_back(vec3(-1, 0, 0));
        s_ArrowNorm[ARROW_CONE_CAP].push_back(vec3(-1, 0, 0));
        s_ArrowNorm[ARROW_CONE_CAP].push_back(vec3(-1, 0, 0));
    }
...
void ArrowWidget::quatFromDirection(quat& out, const vec3& dir) 
{
    float dn = length(dir);
