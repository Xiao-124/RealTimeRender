...
{
	//float TmpData[4];
	return ImGui::SliderFloat4(vLable.c_str(), &vioValue[0], vMin, vMax);
	//vioValue = glm::vec4(TmpData[0], TmpData[1], TmpData[2], TmpData[3]);
}

bool IGUI::sliderFloat(const std::string& vLable, float* v, float v_min, float v_max)
{
...
{
	return ImGuiExt::DirectionWidget(vLable.c_str(), &v[0]);
}
