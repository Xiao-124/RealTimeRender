...
void DecodeSHCoefficientFromVoxelFloat(inout vec3 c[9], int probeIndex)
{
    ...
    for(int i=0; i<9; i++)
    {
        c[i].x = _coefficientSH9Float[offset + i*3+0];
        c[i].y = _coefficientSH9Float[offset + i*3+1];
        c[i].z = _coefficientSH9Float[offset + i*3+2];
    }
}
...
void main()
{
    ...
    vec3 diffuse = gi*_GIIntensity;
    Color_ = vec4(diffuse,1) ;
}