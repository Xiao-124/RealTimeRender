...
vec3 GetSunLuminance(vec3 WorldPos, vec3 WorldDir, float PlanetRadius)
{
	...
	if (dot(WorldDir, sun_direction) > cos(0.5*0.505*3.14159 / 180.0))
	{
		...
		{
			const vec3 SunLuminance = vec3(1000000.0f); // arbitrary. But fine, not use when comparing the models
...
uniform sampler3D AtmosphereCameraScatteringVolume;
...
void main()
{
	...
	//{
	//	output.Luminance = vec4(MultiScatTexture.SampleLevel(samplerLinearClamp, pixPos / vec2(512, 512), 0).rgb, 1.0);
	//	return output;
	//}
	...
	//vec2 uv = pixPos / vec2(1280.0, 720.0);
	DepthBufferValue = textureLod(ViewDepthTexture, v2f_TexCoords, 0).r;
	...
	if (viewHeight < Atmosphere.TopRadius && DepthBufferValue == 1.0f)
	{
		...
		Luminance = vec4(textureLod(SkyViewLutTexture, pixPos / vec2(gResolution), 0).rgb + GetSunLuminance(WorldPos, WorldDir, Atmosphere.BottomRadius), 1.0);
		//FragColor = vec4(textureLod(SkyViewLutTexture uv, 0).rgb + GetSunLuminance(WorldPos, WorldDir, Atmosphere.BottomRadius), 1.0);
		return;
	}
#else
...
#error The FASTAERIALPERSPECTIVE_ENABLED path does not support COLORED_TRANSMITTANCE_ENABLED.
	...
	{
		// Ray is not intersecting the atmosphere		
		Luminance = vec4(GetSunLuminance(WorldPos, WorldDir, Atmosphere.BottomRadius), 1.0);
		return;
	}
...
#if COLORED_TRANSMITTANCE_ENABLED
	output.Luminance = vec4(L, 1.0f);
	output.Transmittance = vec4(throughput, 1.0f);
#else
	const float Transmittance = dot(throughput, vec3(1.0f / 3.0f, 1.0f / 3.0f, 1.0f / 3.0f));
	Luminance = vec4(L, 1.0 - Transmittance);
#endif
