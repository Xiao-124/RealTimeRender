...
const glm::vec3 LIGHT_COLOR = tempUse.BurlyWood; // CHANGE AREA LIGHT COLOR HERE!
bool keys[1024]; // activated keys
glm::vec3 areaLightTranslate;
Shader* ltcShaderPtr;
Shader* planeShaderPtr;
// camera
...
{
    static glm::vec3 color = tempUse.SlateGray;
    static float roughness = 0.5f;
    roughness += step;
    roughness = glm::clamp(roughness, 0.4f, 1.0f);
    //std::cout << "roughness: " << roughness << '\n';
    ltcShaderPtr->use();
    ltcShaderPtr->setVec4("material.albedoRoughness", glm::vec4(color, roughness*roughness));
    glUseProgram(0);
}
...
void incrementLightIntensity(float step)
{
    ...
    glUseProgram(0);
    planeShaderPtr->use();
    planeShaderPtr->setFloat("intensity", intensity);
    glUseProgram(0);
}
...
int main()
{
    ...
    // SHADERS
    Shader shaderLTC("area_light.vs", "area_light.fs");
    ltcShaderPtr = &shaderLTC;
    Shader shaderLightPlane("light_plane.vs", "light_plane.fs");
    planeShaderPtr = &shaderLightPlane;
    // TEXTURES
    unsigned int concreteTexture = loadTexture("../Texture/concreteTexture.png", true);


    unsigned int FBOTextureArray0 = loadTexture("../Texture/0.png", true);
    //unsigned int FBOTextureArray1 = loadTexture("../Texture/1.png", true);
    //unsigned int FBOTextureArray2 = loadTexture("../Texture/2.png", true);
    //unsigned int FBOTextureArray3 = loadTexture("../Texture/3.png", true);
    //unsigned int FBOTextureArray4 = loadTexture("../Texture/4.png", true);
    //unsigned int FBOTextureArray5 = loadTexture("../Texture/5.png", true);
    //unsigned int FBOTextureArray6 = loadTexture("../Texture/6.png", true);
    //unsigned int FBOTextureArray7 = loadTexture("../Texture/7.png", true);
    //unsigned int FBOTextureArray8 = loadTexture("../Texture/8.png", true);
    ...
    unsigned int u_FilteredLightTexture;
    glGenTextures(1, &u_FilteredLightTexture);
    glBindTexture(GL_TEXTURE_2D_ARRAY, u_FilteredLightTexture);
    ...
    for (int i = 0; i < 9; i++)
    {
        ...
        if (data)
        {
            ...
            if (i == 0)
            {
                glTexImage3D(GL_TEXTURE_2D_ARRAY, 0, internalFormat, width, height, 8, 0, internalFormat, GL_UNSIGNED_BYTE, nullptr);
            }
            glTexSubImage3D(GL_TEXTURE_2D_ARRAY, 0, 0, 0, i, width, height, 1, internalFormat, GL_UNSIGNED_BYTE, data);
    ...
    shaderLTC.setVec3("areaLight.points[0]", areaLightVertices[4].position);
    shaderLTC.setVec3("areaLight.points[1]", areaLightVertices[5].position);
    shaderLTC.setVec3("areaLight.points[2]", areaLightVertices[0].position);
    shaderLTC.setVec3("areaLight.points[3]", areaLightVertices[1].position);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        shaderLTC.setInt("LTC2", 1);
        shaderLTC.setInt("material.diffuse", 2);
        shaderLTC.setInt("u_FilteredLightTexture", 3);
        ...
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, FBOTextureArray0);
        renderAreaLight();
        glUseProgram(0);
