...
bool BloomRenderer::Init(unsigned int windowWidth, unsigned int windowHeight)
{
    ...
    // Shaders
    mDownsampleShader = new Shader("new_downsample.vs", "new_downsample.fs");
    mUpsampleShader = new Shader("new_upsample.vs", "new_upsample.fs");
...
int main()
{
    ...
    Shader shaderBlur("old_blur.vs", "old_blur.fs");
    Shader shaderBloomFinal("bloom_final.vs", "bloom_final.fs");

    // load textures
    // -------------
    unsigned int woodTexture = loadTexture("../Textures/wall.jpg", true); // note that we're loading the texture as an SRGB texture
    unsigned int containerTexture = loadTexture("../Texture/wall.jpg", true); // note that we're loading the texture as an SRGB texture
